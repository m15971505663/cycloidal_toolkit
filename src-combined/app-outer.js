// 外摆应用模块（集成版）：源自 src-outer/main.js，工厂化 —— root 作用域 + 隐藏挂起 + 共享徽章
window.createOuterApp = function (root, ui) {
"use strict";
/* ============================================================
   外摆线减速器 · 原理演示（小红书小工具）
...
function resize() {
  ...
  if (r.width < 10) return;                 // pane 隐藏时挂起（保持上次尺寸）
...
// ---- 渲染 ----
function draw() {
  if (!root.offsetWidth) return;           // pane 隐藏时跳过绘制
  const p = P();
  const S = stageIndex;
...
function applyParams() {
  ...
  ui.badge.textContent = '1 : ' + (state.zc + 1);
...
function setStage(i) {
  stageIndex = i;
  root.querySelectorAll('.stage-dot').forEach((d, k) => d.classList.toggle('active', k === i));
...
root.querySelectorAll('.stage-dot').forEach(d => {
...
loop();
  ...
  return { resize: resize, draw: draw, applyParams: applyParams, setStage: setStage };
};
